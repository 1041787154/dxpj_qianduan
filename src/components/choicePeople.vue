					...
					<div class="contentBox_2">
						<div class="message">
							<p>
							 ...
							 <p>
                        <span>类别 :</span><select name="PartyPosition" id="PartyPosition" @change="changeType">
                          <option style="width:100px" value="党员领导干部" >党员领导干部</option>
                          <option style="width:100px" value="机关党员">机关党员</option>
                          <option style="width:100px" value="两新组织党员">两新组织党员</option>
                          <option style="width:100px" value="居民党员">居民党员</option>
                        </select>
                    </p>
							<p>
...
<script>import quesDia_one from "./quesDia_one";
...
export default {
	...
	methods: {
		...
		choose(row, rowData, column) {
			this.nowChoose = row;
			this.password = "";
			this.video(this.audio_type);
			console.log('--------choose--------:' + this.bool_audioOpen);
		},
		chooseNum(num) {
			if(num != -1) {
				this.password += num;
		...
		next(url_) {
			var myselect = document.getElementById("PartyPosition");
				...
				this.$ajax({
					...
					.then(res => {
						...
						if(res.status.code == 100) {
							...
							this.password = "";
							this.showPopOne(res.status.msg);
							this.video(res.status.msg);//验证失败
						}
					})
					.catch(function(err) {
						console.log(err);
	...
	created: function() {
		...
		this.$ajax({
			...
			.then(res => {
				...
				if(res.status.code == 100) {
					var list = res.info;
					this.video(res.info_B_determine);
					this.audio_type = res.info_C_type;
					this.audio_pass = res.info_C_pass;
