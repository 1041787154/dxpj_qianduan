	...
	<div class="myBody_">
		...
		<div class="showDia_redLine" v-if="dia_redLine">
			<div class="child_contenter">
				<redLineDia :sendToRed="sendToRed" v-if="sendToRed" @get-data="getRed" @backFromRed="backFromRed"></redLineDia>
			</div>
		</div>

		<div class="dia_ques" v-if="bool_diaTwo">
			<quesDia :messageTwo="messageTwo" v-if="messageTwo" @noShoWRed="noShoWRed" @showRed="showRed" @quitAll="quitAll" @noQuitAll="noQuitAll" @updateFromQuit="updateFromQuit" @noUpdateFromQuit="noUpdateFromQuit" @saveOne="saveOne" @noSaveOne="noSaveOne" @updateAll="updateAll" @noUpdateAll="noUpdateAll" @updateAllAndQuit="updateAllAndQuit" @noUpdateAndQuit="noUpdateAndQuit"></quesDia>
		</div>
	...
	export default {
		...
		closeAudio() {
										...
										if(
											...
											for(var i = 0; i < this.items_ques.length; i++) {
												if(this.info_record[k]["Answer_" + i] != "") {
													...
													this.flag[i] = true;
													fa = i;
													this.items_ques[i].number = this.info_record[k]["Answer_" + i];
												}
											}
											...
											for(var j = 0; j < this.items_red.length; j++) {
												if(this.info_record[k][this.items_red[j].RedLineID] != "") {
													num_redFinished++;
													this.items_red[j].number = this.info_record[k][this.items_red[j].RedLineID];
												}
											}
			...
			getRed(childValue) {
				...
				this.flag_red = this.judgeRedRecord();
				...
				this.$ajax({
					...
					.then(res => {
						...
						if((res.code = 100)) {
							...
							this.bool_isUpdated = true;
							this.video(this.messageM);
				...
				this.$ajax({
					...
					.then(res => {
						...
						if((res.code = 100)) {
							...
							this.showPopOne(this.messaegP);//"提交成功!即将跳转首页！"
			...
			update() {
				//				if(!this.num_finish()) {
				//					this.showPopOne('未完成80%的评价，请您继续评价！');
				//				} else {
			...
			turn(type) {
				if(type == -1) {
					...
					if(this.items_ques[this.nowquest.DisplayOder - 1].number) {
						...
						this.showPopOne(this.messageV);//请先完成本题答案
					}
				}
			},
			saveOne() {
			...
			this.$ajax({
				...
				.then(res => {
					...
					if(res.status.code == 100) {
						...
						this.member_assessed = res.list_member_Assessed;
                        ...
                        this.messageQ = res.info_C_determine;// "请您按照屏幕显示的党员姓名依次输入各党员的组织评议分值，请选择你要评议的党员",
...
//						this.showPopOne(this.messageG);
		...
		data() {
			return {
				...
				messageQ:  "请您按照屏幕显示的党员姓名依次输入各党员的组织评议分值，请选择你要评议的党员",
